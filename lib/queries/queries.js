...
Query.viewRoles = () => {
    db.query("SELECT role.id, role.title, role.salary, department.name AS department FROM role JOIN department ON role.department_id=department.id;", (err, res) => {
        if (err) {
            console.log(err);
        }
        console.table(res);;
...
Query.viewEmployees = () => {
    db.query("SELECT employee.id, employee.first_name, employee.last_name, role.title AS role, role.salary, employee.manager_id FROM employee JOIN role ON employee.role_id=role.id;", (err, res) => {
        if (err) {
            console.log(err);
        }
        console.table(res);
    ...
    db.query(`SELECT name FROM department;`, (err, res) => {
        ...
        inquirer
            ...
            .then((response) => {
                ...
                db.query(`INSERT INTO role (title, salary, department_id) VALUES ("${newRole}", "${salary}", "${departmentNo}");`, (err, res) => {
                    if (err) {
                        console.log(err);
                    }
        ...
        db.query(`SELECT id, title FROM role;`, (err, res) => {
            ...
            inquirer
                ...
                .then((response) => {
                    ...
                    db.query(`UPDATE employee SET role_id = ${newRoleID} WHERE id = ${employeeID}`, (err, res) => {
                        if(err) {
                            console.log(err);
                        }
